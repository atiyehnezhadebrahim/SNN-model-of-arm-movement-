...
class connect():
    ...
    def create_connections(self,pop1,pop2,syn_type):
        ...
        self.strong_inh_syn_params = {"synapse_model":"static_synapse",
            ...
            "delay" : netparams.synaptic_delay}	#ms
        self.weak_exc_syn_params = {"synapse_model":"static_synapse",
            "weight" : nest.random.normal(mean=netparams.w_weak_exc_mean,std=netparams.w_weak_exc_std), #nS            
            "delay" : netparams.synaptic_delay}	#ms       
        
        if syn_type=='exc':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_rg,self.exc_syn_params) 
        if syn_type=='inh':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_rg,self.inh_syn_params)
        if syn_type=='exc_weak':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_rg,self.weak_exc_syn_params)    
        if syn_type=='inh_strong':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_cpg,self.strong_inh_syn_params)
        if syn_type=='exc_all':
            nest.Connect(pop1,pop2,'all_to_all',self.exc_syn_params)         
        if syn_type=='inh_all':
            nest.Connect(pop1,pop2,'all_to_all',self.inh_syn_params)
        if syn_type=='exc_mn':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_mn,self.exc_syn_params)           
        if syn_type=='inh_strong_mn':
            nest.Connect(pop1,pop2,netparams.conn_dict_custom_mn,self.strong_inh_syn_params)    
    
    def create_gap_connections(self,pop1,pop2):
        nest.Connect(pop1,pop2,netparams.conn_dict_custom_gap,syn_spec={"synapse_model": "gap_junction","weight": netparams.init_gap_weight})
        gap_conn = nest.GetConnections(pop1,pop2,synapse_model="gap_junction")
        return gap_conn
